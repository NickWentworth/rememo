...
export function getTimeDivider(task) {
    ...
    // gets weekday from monday(0) to sunday(6)
    let todayWeekday = (today.getDay() + 6) % 7;

    let overdue = due < today; // detecting if due date has already passed
    let daysAway = Math.round((due.setHours(0, 0, 0) - today.setHours(0, 0, 0)) / msPerDay); // number of days from today

    let divider = '';
    console.log(`Today: ${todayWeekday}, Days Away: ${daysAway}, Total: ${todayWeekday + daysAway}`);
    switch (true) {
        case (overdue):
            divider = 'Overdue';
            break;
        case (todayWeekday + daysAway <= 4):
            divider = 'Due this week';
            break;
        case (todayWeekday + daysAway <= 6):
            divider = 'Due this weekend';
            break;
        case (todayWeekday + daysAway <= 11):
            divider = 'Due next week';
            break;
        case (todayWeekday + daysAway <= 13):
            divider = 'Due next weekend';
            break;
        default:
            divider = '2+ weeks away';
            break;
    }
    return divider;
} 
